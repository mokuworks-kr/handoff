/**
 * wide-narrow-text-image — 넓은 텍스트 + 좁은 이미지 콤포지션.
 *
 * 페이지를 비대칭 2분할 (default 어휘 [8,4]). 넓은 칸 텍스트, 좁은 칸 이미지.
 *
 ...
 *   1차 어휘 [8,4] 기준. 미래에 다른 비대칭 비율(7-5, 9-3) 어휘 추가 시 콤포지션 따로 추가.
 * - **variants — asymmetryDirection** (★): 비대칭 콤포지션의 핵심 결정.
 ...
 * - **totalRows = 12**: 통일.
 *
 * variants override 설명:
 *   wide-left  (default): wide=col 1 span 8, narrow=col 9 span 4
 *   wide-right          : wide=col 5 span 8, narrow=col 1 span 4
 *   검증: 8+4 = 12 (default 어휘 [8,4]) — §11 약속 1번 안 깸.
 */

import type { CompositionPattern } from "@/lib/layout/composition";
import { buildAsymmetryVariants } from "./_variants";

export const wideNarrowTextImage: CompositionPattern = {
  slug: "wide-narrow-text-image",
  name: "넓은 텍스트 + 좁은 이미지",
  description:
    "페이지를 비대칭 2분할 (8:4). 넓은 칸 텍스트, 좁은 칸 이미지. 제품 카탈로그(제품명+소개글 / 제품 이미지), 본문+사이드 등에 사용.",
  role: "body",
  totalRows: 12,
  slots: [
    {
