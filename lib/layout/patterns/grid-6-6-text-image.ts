/**
 * grid-6-6-text-image — 텍스트 + 이미지 균등 콤포지션.
 *
 * ─────────────────────────────────────────────────────────────
 * 무엇 / 왜
 * ─────────────────────────────────────────────────────────────
 *
 * 페이지를 6칸 + 6칸으로 분할. 한쪽은 텍스트, 한쪽은 이미지.
 * 회사소개서 / IR 에서 매우 빈번:
 *   - 회사 소개 단락 + 사옥 사진
 *   - 비전 본문 + 컨셉 이미지
 ...
 *   좌우 반전이 필요해지면 미래에 별도 콤포지션 grid-6-6-image-text 추가
 *   (어휘 [6,6] 안의 변형이라 §11 약속 1번 안 깸. 위험 7 갈래 C 의 일관된 적용).
 *
 *   왜 좌우를 variants 로 안 두나: [6,6] 은 대칭 비율이라 _variants.ts 의
 ...
 *   카탈로그 사용자 케이스 ("제품명+소개글 / 제품 이미지") 와 자연스럽게 일치.
 *
 * - **totalRows = 12**: 통일.
 */
...
export const grid66TextImage: CompositionPattern = {
  slug: "grid-6-6-text-image",
  name: "텍스트 + 이미지",
  description:
    "페이지를 6칸 + 6칸으로 분할. 왼쪽 텍스트, 오른쪽 이미지. 회사 소개+사옥, 인물 약력+사진, 제품 설명+이미지 등에 사용.",
  role: "media",
  totalRows: 12,
  slots: [
    {
