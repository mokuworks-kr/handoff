...
intentionalOmissions: 의도적으로 본문에서 제외한 블록 (있을 때만)
...
## 6. 모든 블록은 어딘가로 가야 합니다 (절대 원칙)

manuscript.blocks 의 **모든 블록**이 다음 두 곳 중 하나에 있어야 합니다:

1. 어느 페이지의 slotBlockRefs 안 (정상 배치) — **이게 압도적 다수**
2. \`intentionalOmissions\` 안 (명시적 누락, reason 포함)

세 번째 경로는 없습니다. 침묵 누락은 검증에서 잡혀 페이지네이션 전체가 실패합니다.

특히 separator 블록(\`type: "separator"\`)도 마찬가지입니다 — 페이지 분할 신호로 사용한 후 \`intentionalOmissions\` 에 \`reason: "페이지 구분 마커, 본문에 표시 X"\` 식으로 명시.

마지막 출력 전 자가 점검:
- manuscript.blocks 의 ID 를 모두 모은다 (b0001, b0002, ..., bNNNN)
- 출력 페이지들의 slotBlockRefs 의 모든 값을 평탄화한다
- intentionalOmissions 의 blockIds 를 더한다
- 두 집합이 같은가? 다르면 침묵 누락이 있는 것.
...
- b0003 [separator page] (작성자가 여기서 페이지 끊음)
- b0004 [heading] "회사 개요"
...
- b0010 [separator page]
- b0011 [heading] "재무 요약"
- b0012 [table 4×3] (분기별 매출표)
- b0013 [paragraph] "표 설명 — 본문 우측 narrow 슬롯에"
...
\`\`\`json
{
  "pages": [
    ...
    {
      "pageNumber": 4, "pattern": "wide-narrow-table-text", "role": "data", "side": "left",
      "variants": { "asymmetryDirection": "wide-left" },
      "slotBlockRefs": { "wide": ["b0012"], "narrow": ["b0011", "b0013"] },
      "splitReason": "page-separator",
      "rationale": "재무 표 + 우측 설명"
    }
  ],
  "intentionalOmissions": [
    {
      "blockIds": ["b0003", "b0007", "b0010"],
      "reason": "페이지 구분 마커 (separator) — 페이지 경계 신호로만 사용, 본문 표시 X"
    }
  ]
}
\`\`\`
...
- 모든 manuscript 블록(b0001~b0013)이 어딘가에 있음
- b0003/b0007/b0010 (separator 페이지 마커) 는 intentionalOmissions 에 명시됨
- 페이지 4 는 다중 슬롯 콤포지션 — wide(table) + narrow(text) 둘 다 채워짐
...
- 모든 블록이 어떤 페이지의 slotBlockRefs 에 들어갔거나, intentionalOmissions 에 명시됐나? (§6)
