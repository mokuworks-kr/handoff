...
manuscript 의 모든 블록이 어딘가의 페이지에 들어가야 합니다. 침묵 누락(아무 페이지에도 안 넣고 침묵)은 검증에서 잡힙니다.

본문에서 제외할 의도가 명확한 블록(예: 부록, 표지에 안 들어갈 메타데이터)은 \`intentionalOmissions\` 에 reason 과 함께 명시.
...
- 모든 블록이 어떤 페이지에 들어갔거나 intentionalOmissions 에 명시됐나?
